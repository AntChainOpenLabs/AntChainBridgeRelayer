...
public class AuthMsgPackage {
    ...
    public static AuthMsgPackage convertFrom(List<SDPMsgWrapper> sdpMsgWrappers, PTCProofResult ptcProofResult) {
        ...
        sdpMsgWrappers.forEach(
                sdpMsgWrapper -> authMsgPackage.addAmMsg(
                        ...
                        sdpMsgWrapper.getAuthMsgWrapper().getAuthMessage().encode()
                )
        );
    ...
    public static byte[] extractProofs(byte[] encodedAuthMsgPackage) {
        int offset = RECEIVER_IDENTITY_BYTEARRAY_LEN + SENDER_DOMAIN_LEN + FLAGS_LEN;
        int proofs_len = encodedAuthMsgPackage.length - offset;
        byte[] proofs = new byte[proofs_len];
        System.arraycopy(encodedAuthMsgPackage, offset, proofs, 0, proofs_len);
        return proofs;
    }
